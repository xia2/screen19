...
filelist = [
    f for f in os.listdir(".") if os.path.isfile(f) and fnmatch.fnmatch(f, "*.cbf")
]
...
def test_wavelength(data, straight_return=False):
    ...
    if m:
        ...
        if wl < 0.65 or wl > 0.95:
            raise Exception(
                "Wavelength according to header is %f A. Is this correct?" % wl
            )
        return
...
def test_gain(data):
    ...
    def test_values(exp_tau, exp_gain, exp_vrf):
        ...
        if abs((tau - exp_tau) / exp_tau) > 0.1:
            e.append(
                "Tau (%e) deviates from expected tau (%e) by more than 10%%"
                % (tau, exp_tau)
            )
        if abs((vrf - exp_vrf) / exp_vrf) > 0.1:
            e.append(
                "Vrf (%e) deviates from expected vrf (%e) by more than 10%%"
                % (vrf, exp_vrf)
            )
        return e
    ...
    if (abs(expected_threshold - actual_threshold) / expected_threshold) > 0.1:
        raise Exception(
            "Threshold (%f eV) deviates from expected threshold (%f eV) by more than 10%%"
            % (actual_threshold, expected_threshold)
        )
    if actual_threshold < 4000:
        raise Exception(
            "Threshold (%f eV) outside detector specification (< 4 keV)"
            % actual_threshold
        )
        ...
        raise Exception(
            "Threshold (%f eV) outside detector specification (> 18 keV)"
            % actual_threshold
        )
